...
interface RangeBuilderPopupProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export default function RangeBuilderPopup({ isOpen, onClose }: RangeBuilderPopupProps) {
  ...
  // Выбор всех рук (все 169 комбинаций)
  ...
  // Загрузка сохраненного диапазона
  ...
  // Удаление сохраненного диапазона
  ...
  const popupContent = (
    ...
    >
      <div
        className="bg-gradient-to-br from-slate-800 to-slate-900 border-2 border-slate-700 rounded-xl shadow-2xl p-4 relative z-[10000] max-w-7xl w-full max-h-[95vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Заголовок */}
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-lg font-bold text-white">Конструктор диапазонов</h3>
            <p className="text-xs text-gray-400 mt-1">
              Создавайте и экспортируйте диапазоны в JSON формате
            </p>
          </div>
          <button
            onClick={onClose}
            className="text-gray-400 hover:text-white transition-colors text-sm font-bold w-8 h-8 flex items-center justify-center rounded hover:bg-white/10"
          >
            ✕
          </button>
        </div>
