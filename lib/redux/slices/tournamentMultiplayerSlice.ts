/**
 * Redux slice для Tournament Multiplayer (Турнирная игра с друзьями)
 */

import { createSlice, PayloadAction } from "@reduxjs/toolkit";
import type {
  TournamentMultiplayerState,
  TournamentRoom,
  TournamentMultiplayerPlayer,
  TournamentRoomSettings,
} from "../types/tournamentMultiplayerTypes";
...
// Начальное состояние
const initialState: TournamentMultiplayerState = {
  currentRoom: null,
  availableRooms: [],
  currentUserId: null,
...
const tournamentMultiplayerSlice = createSlice({
  name: "tournamentMultiplayer",
  initialState,
  reducers: {
    /**
    ...
    createRoom: (
      ...
      const { roomName, maxPlayers = 8 } = action.payload;
      ...
      const hostPlayer: TournamentMultiplayerPlayer = {
      ...
      const newRoom: TournamentRoom = {
        ...
        maxPlayers,
        createdAt: Date.now(),
        settings: getDefaultTournamentSettings(),
      };
    ...
    joinRoom: (state, action: PayloadAction<{ roomId: string }>) => {
      ...
      // Создаем игрока
      const newPlayer: TournamentMultiplayerPlayer = {
        id: state.currentUserId,
        name: state.currentUserName,
     ...
     */
    setAvailableRooms: (state, action: PayloadAction<TournamentRoom[]>) => {
     ...
     */
    setCurrentRoom: (state, action: PayloadAction<TournamentRoom>) => {
...
export const {
  ...
  setCurrentRoom,
  clearCurrentRoom,
} = tournamentMultiplayerSlice.actions;

export default tournamentMultiplayerSlice.reducer;
